...
public class PluginInitializerManager {
    ...
    public static void load(OptionSet optionSet) throws Exception {
        ...
        final String ICON_SUCCESS = "✅";
        final String ICON_LOADING = "⏳";

        LOGGER.info(ANSI_GREEN + ICON_LOADING + "  Inicializando plugins.." + ANSI_RESET);
        // We have to load the bukkit configuration inorder to get the update folder location.
        ...
        LOGGER.info(ICON_SUCCESS + " " + ANSI_GREEN + " Inicializado" + ANSI_RESET + " " + total + " plugin" + (total == 1 ? "" : "s"));

        if (!paperPluginNames.isEmpty()) {
            String paperPrefix = ICON_SUCCESS + " " + ANSI_GREEN + " Paper plugins" + ANSI_RESET + " (" + paperPluginNames.size() + "):";
            LOGGER.info(paperPrefix);

            if (LOGGER.isDebugEnabled()) {
                // Lista detalhada, cada plugin numa linha com " - "
                for (String plugin : paperPluginNames) {
                    LOGGER.info(" - " + plugin);
                }
            } else {
                // Lista simples, plugins separados por vírgula na mesma linha
                LOGGER.info(" " + String.join(", ", paperPluginNames));
            }
        }

        if (!legacyPluginNames.isEmpty()) {
            String bukkitPrefix = ICON_SUCCESS + " " + ANSI_GREEN + " Bukkit plugins" + ANSI_RESET + " (" + legacyPluginNames.size() + "):";
            LOGGER.info(bukkitPrefix);

            if (LOGGER.isDebugEnabled()) {
                for (String plugin : legacyPluginNames) {
                    LOGGER.info(" - " + plugin);
                }
            } else {
                LOGGER.info(" " + String.join(", ", legacyPluginNames));
            }
        }
    }
